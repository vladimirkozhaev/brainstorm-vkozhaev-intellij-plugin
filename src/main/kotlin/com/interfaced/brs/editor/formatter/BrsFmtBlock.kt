...
import com.interfaced.brs.lang.psi.BrsCatchStmt
import com.interfaced.brs.lang.psi.BrsEndTry
import com.interfaced.brs.lang.psi.BrsTryStmt
    ...
    companion object {
        ...
        fun computeIndent(child: ASTNode, node: ASTNode): Indent? {
            ...
            return when (parentPsi) {
                is BrsFunctionStmtImpl -> {
                    indentIf(
                        childPsi !is BrsEndFunctionImpl
                                && childPsi !is BrsFnDeclImpl
                    )
                }
                is BrsSubStmtImpl -> {
                    indentIf(
                        childPsi !is BrsEndSubImpl
                                && childPsi !is BrsSubDeclImpl
                    )
                }
                is BrsIfStmtImpl -> {
                    indentIf(
                        childPsi !is BrsEndIfImpl
                ...
                is BrsTryCachStmtImpl -> {
                    indentIf(childPsi !is BrsTryStmt && childPsi !is BrsEndTry && childPsi !is BrsCatchStmt)
                }
                else -> Indent.getNoneIndent()
            }
        }
